...
renderDearImGui :: ImGuiResources -> FrameContext -> IO () -> IO ()
renderDearImGui ImGuiResources {..} FrameContext {..} f = do
  vulkanNewFrame
  glfwNewFrame
  newFrame
...
initDearImGui window vulkanResources@VulkanResources {..} Swapchain {..} = do
  ...
  _ <- mkAcquire (glfwInitForVulkan window True) (const glfwShutdown)
