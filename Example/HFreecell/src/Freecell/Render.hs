    ...
    where cursorPos = unproject cursor (-5) mat ss :: V3
          renderCard card = let tid = HashMap.lookup card cardTexHash
                                ...
                                pos = case sel of
                                          ...
                                          Nothing -> cardPos
                                in Primative nillaSh (sizePosMat (Size 1 1) pos) white (Square tid)
          cards = map renderCard allCards
          piles = map (\pos -> Primative nillaSh (sizePosMat (Size 1 1) (v2tov3 pos (-40))) white (Square (Just blankTex))) (drop 8 pilePositions)
