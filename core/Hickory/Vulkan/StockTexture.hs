...
import Vulkan (SamplerAddressMode (..), Filter (..), ImageAspectFlagBits (..), Format (..), SamplerMipmapMode (..))
...
withWhiteImageDescriptor vr@VulkanResources {..} = do
  ...
  image <- fst <$> withImageFromArray vr 1 1 format False (SV.fromList [255 :: PixelBaseComponent PixelRGBA8, 255, 255, 255])
